...
import numpy as np
from keras.layers import Lambda, Conv2D, MaxPooling2D, Activation, Dense, Dropout, Flatten
...
def recon_lambda(x, means, convs, divider, p, randMat):    
    ...
    out = tf.nn.conv2d_transpose(out, means+randMat, 
...
def get_classmodel(data, model_path, layer_path):
    ...
    randMat = np.random.normal(0, 1.75, [shp[0],shp[1],shp[2],shp[3]]).astype(np.float32)
    randMat = randMat *convs
    _args = {'means':means, 'convs': convs, 
             'divider': get_divider(data.train_data.shape[1]),  
             'p': 25,
             'randMat': randMat}
    ...
    model.add(Conv2D(params[0], (3, 3), padding='same', input_shape=data.train_data.shape[1:]))
    ...
    model.add(Activation('relu'))
    model.add(Conv2D(params[3], (3, 3), padding='same'))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
...
def get_normal_acc():
    total_acc = get_acc(data.test_data, data.test_labels)
...
def get_attack_acc(fileName):
    ...
    total_acc = get_acc(test_data, test_labels)
    print(fileName, '  total_acc : ', total_acc )
...
''' Check/ update the parameters before executing.'''
