...
def EStep(config, updateMode, batch, nodeMean, nodeCov, 
    ...
    if updateMode < 2:
        ...
        outBatch[:,:,:,-1] = Globals.alpha*outBatch[:,:,:,-1]
    if updateMode == 2:
...
def updateParam(statMean, statDigCov, clusterCount, nodeMean, nodeCov):
    ...
    for f in range(0, nodeMean.shape[0]):
        if clusterCount[f] > Globals.countThreshold:
            if (f<nodeMean.shape[0]-1) or (f==nodeMean.shape[0]-1 and clusterCount[f]/countSum > Globals.memberThreshold):
