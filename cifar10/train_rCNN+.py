...
class cifar10vgg:
    ...
    def train(self, model, data, model_path, existing_path = None):
        ...
        '''
        dataList1 = np.load("./extras/bim_05.npz")
        diff_data1 = dataList1['x_test']- data.train_data
        diff_gen1 = diffgen1.flow(diff_data1, y_train, batch_size=batch_size)
        ...
        def flow_noisy(train_gen):
            ...
            while True:
                ...
                # noisy data gen
                in_rand = np.random.normal(0,0.03,[b_size, 32, 32, 3]).astype(np.float32)
                x2 = x0 + in_rand
...
if __name__ == '__main__':    
    ...
    model = cifar10vgg()
    model_path = './trained_models/rCNN+_2'
    vgg = model.train(model= model.model, data= data, 
