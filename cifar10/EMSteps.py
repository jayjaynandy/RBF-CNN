...
def EStep(config, updateMode, batch, nodeMean, nodeCov, statMean, statDigCov, clusterCount):
    ...
    if updateMode < 2:
        ...
        outBatch[:,:,:,-1] = Locals.alpha*outBatch[:,:,:,-1]
    if updateMode == 2:
...
def updateParam(statMean, statDigCov, clusterCount, nodeMean, nodeCov):
    countSum = np.sum(clusterCount)
    countNode = 0
    for f in range(0, nodeMean.shape[0]):
        if clusterCount[f] > Locals.countThreshold:
            if (f<nodeMean.shape[0]-1) or (f==nodeMean.shape[0]-1 and clusterCount[f]/countSum > Locals.memberThreshold):
